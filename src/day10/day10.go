...
func TaskTwo() {
	...
	loop := getMainLoop(pipes)
	var insideLoop []tile
	var outsideLoop []tile

	for rowIndex, row := range pipes {
		for columnIndex, column := range row {
			curr := tile{
				x:    columnIndex,
				y:    rowIndex,
			...
			populate(loop, insideLoop, outsideLoop, curr)
		}
	}
}

func populate(loop []step, insideLoop []tile, outsideLoop []tile, curr tile) {
	//isLoop
	for _, loopPipe := range loop {
		if curr == loopPipe.tile {
			return
		}
	}

	//alreadyFound outside
	for _, outSideLoopTile := range outsideLoop {
		if curr == outSideLoopTile {
			return
		}
	}

	//alreadyFound inside
	for _, insideSideLoopTile := range insideLoop {
		if curr == insideSideLoopTile {
			return
		}
	}

	//TODO check Neighbours recursively
